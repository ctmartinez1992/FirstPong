...
#define VELOCIDADE_RAQUETES 1
#define TAMANHO_BOLA        10
#define DELAY_MOVIMENTO     10
#define DELAY_TECLAS        8
...
void inicia_jogo() {
	...
	modelo.bola.direccao=0;//RAD(rand()%120-60);
	if(modelo.servico==2)
...
void Timer(int value)
{
	...
	//Testa fim de ponto
	if(modelo.bola.x<=modelo.jogador1.x-10) {
		modelo.jogador2.pontuacao++;
		inicia_jogo();
	}
	if(modelo.bola.x>=modelo.jogador2.x+10) {
		modelo.jogador1.pontuacao++;
		inicia_jogo();
	}

	//Testa colis�o paredes
	if((modelo.bola.y+modelo.bola.tamanho/2)>=ALTURA_CAMPO)
		modelo.bola.direccao=-modelo.bola.direccao;
	if((modelo.bola.y-modelo.bola.tamanho/2)<=0)
		modelo.bola.direccao=-modelo.bola.direccao;

	//Colis�o jogador 1
	if((modelo.jogador1.y+(modelo.alturaRaquetes/2))>=ALTURA_CAMPO)
		modelo.jogador1.y=ALTURA_CAMPO-modelo.alturaRaquetes/2;
	if((modelo.jogador1.y-(modelo.alturaRaquetes/2))<=0)
		modelo.jogador1.y=modelo.alturaRaquetes/2;
	if(modelo.bola.x<=modelo.jogador1.x+LARGURA_RAQUETES && modelo.bola.y<=(modelo.jogador1.y+modelo.alturaRaquetes/2) && modelo.bola.y>=(modelo.jogador1.y-modelo.alturaRaquetes/2)) {	
		if(estado.delayMovimento>3) {
			estado.delayMovimento-=0.5;
			estado.delayTeclas-=0.5;
			modelo.velocidadeRaquetes+=0.3;
			modelo.bola.velocidade+=0.4;
		}
		float diff = modelo.bola.y-modelo.jogador1.y;
		modelo.bola.direccao=M_PI-modelo.bola.direccao+RAD(diff*0.75);
	}

	//Colis�o jogador 2
	if((modelo.jogador2.y+(modelo.alturaRaquetes/2))>=ALTURA_CAMPO)
		modelo.jogador2.y=ALTURA_CAMPO-modelo.alturaRaquetes/2;
	if((modelo.jogador2.y-(modelo.alturaRaquetes/2))<=0)
		modelo.jogador2.y=modelo.alturaRaquetes/2;
	if(modelo.bola.x>=modelo.jogador2.x-LARGURA_RAQUETES && modelo.bola.y>=(modelo.jogador2.y-modelo.alturaRaquetes/2) && modelo.bola.y<=(modelo.jogador2.y+modelo.alturaRaquetes/2)) {
		if(estado.delayMovimento>3) {
			estado.delayMovimento-=0.5;
			estado.delayTeclas-=0.5;
			modelo.velocidadeRaquetes+=0.3;
		...
		float diff = modelo.bola.y-modelo.jogador2.y;
		modelo.bola.direccao=M_PI-modelo.bola.direccao-RAD(diff*0.75);
	}
...
{
	if(key == GLUT_KEY_UP) estado.teclas.p=GL_TRUE;
	if(key == GLUT_KEY_DOWN) estado.teclas.l=GL_TRUE;
}

void SpecialKeyUp(int key, int x, int y)
{
	if(key == GLUT_KEY_UP) estado.teclas.p=GL_FALSE;
	if(key == GLUT_KEY_DOWN) estado.teclas.l=GL_FALSE;
}
